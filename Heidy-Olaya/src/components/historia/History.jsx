...
export default class History extends Component {
  ...
  componentDidUpdate() {
    console.log("se actualizo componentDidUpdate");
  };
      ...
      <div className="layout">
        ...
        <div className="opciones">
          <div className="opcion">
            <button id="a" className="botones" onClick={this.handleClick}>
              A
            </button>
            <h2>{data[this.state.cont].opciones.a}</h2>
          </div>
          <div className="opcion">
            <button id="b" className="botones" onClick={this.handleClick}>
              B
            </button>
        ...
        <div className="recordatorio">
          <h3>
            Seleccionado anteriormente: {this.state.guardarDato.toUpperCase()}
          </h3>
          <h4>Historial de opciones elegidas</h4>
          <ul>
            {this.state.historialArray.map((h, idx) => (
              <li key={idx}>{h.toUpperCase()}</li>
            ))}
          </ul>
        </div>
      </div>
